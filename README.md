...
ReviewFlow is an incremental rebuild of an existing competitive reputation intelligence application. The existing Supabase data is retained. The change-focused dashboard, read-only history, metric insights, alerts, competitor management, opt-in weekly digest endpoint, and fictional review-analysis demo are implemented. Owners can import recent review text to see repeated topics and supported changes between two 30-day periods. A manual capture pilot is available in Settings. Google Business Profile OAuth and daily own-business capture are implemented but require an approved Google Cloud project and OAuth credentials before activation.
...
On Windows, `next.config.ts` uses one static generation worker to avoid a local worker crash observed when Next.js launched seven workers; Linux deployment keeps its default worker count.
...
The demo uses invented reviews. Automated review-text ingestion still needs an authorized source. Existing Google Places integration is for business discovery; a Google API key alone does not provide complete competitor reviews or establish historical storage rights. Legacy snapshot POST routes are disabled by default with `LEGACY_GOOGLE_CAPTURE_ENABLED=false`.

Signed-in owners can import their own review texts at `/reviews` using a CSV with `fecha,estrellas,texto` columns and an optional final `id` column. The app records a source label and explicit rights declaration, but does **not** verify that declaration or the underlying source. With a stable source ID, importing the same review again updates its text, rating and date; without an ID, identical date, rating and text are deduplicated. Owners can delete one review or all imported reviews. Import accepts only publication dates within the last 90 days, and the daily cron removes older text. This is a 90-day retention limit on stored review text, not a historical archive.

The dashboard analyzes at most 200 own reviews from that window. It displays counts dated in the last seven days and, if there was a prior marked visit, counts dated after that visit's calendar day. Repeated topics require supporting mentions. A temporal topic finding compares the last 30 days with the preceding 30 days only when both periods have at least three reviews, the recent period has at least two matching reviews, and its mention share rose by at least 30 percentage points. If the 200-review cap is exceeded, temporal topic findings are withheld rather than presented as complete. These observations describe the imported sample; they do not prove a cause or represent all customers. Competitor review text remains limited to the fictional demo until there is a permitted source. The local CSV in `tests/fixtures` is fictional.
...
`vercel.json` schedules a Google Business Profile capture at 07:00 UTC and a daily review-text retention/email check at 08:00 UTC in production. The capture job is a no-op until OAuth credentials and the encryption key are configured. The retention check runs independently of the email feature flag. The email job sends each opted-in profile at most once per seven days when there is a recent own-business capture or recent review text in the owner's sample. The message leads with supported findings and next steps, omits raw review quotes, and links to the dashboard for full evidence. The `reviewflow` Vercel project is connected to `Walrus92/reviewflow` and published at `https://project-qdebw.vercel.app`. The migration branch is deployed manually to Production. Automatic Preview builds from this branch currently fail because Preview lacks the production-only Supabase variables; configure an isolated Preview environment before using it for testing.
