...
Signed-in owners can import their own authorized review texts at `/reviews` using a CSV with `fecha,estrellas,texto` columns. Each import needs a source label and explicit rights confirmation. Rows are deduplicated by date, rating and text, saved with provenance, and can be deleted by the owner. The dashboard checks up to 200 reviews published in the last 90 days and shows a topic only after two supporting reviews. This does not establish a licensed source for competitor reviews. The local CSV in `tests/fixtures` is fictional.
