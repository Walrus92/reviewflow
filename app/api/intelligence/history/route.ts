...
export async function GET(request: NextRequest) {
  ...
  const [{ data: own, error: ownError }, { data: relations, error: relationError }] = await Promise.all([
    getSupabaseAdmin().from("review_snapshots").select("id,rating,review_count,created_at,source_kind")
      .eq("profile_id", profileId).eq("source_kind", "manual_owner")
      .order("created_at", { ascending: false }).limit(100),
    getSupabaseAdmin().from("competitor_relations").select("competitor_id")
  ...
  const [{ data: competitors, error: competitorError }, { data: snapshots, error: snapshotError }] = await Promise.all([
    getSupabaseAdmin().from("competitors").select("id,name,place_id").in("id", ids),
    getSupabaseAdmin().from("competitor_snapshots").select("id,competitor_id,rating,review_count,created_at,source_kind")
