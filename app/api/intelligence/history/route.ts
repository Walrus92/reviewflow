...
export async function GET(request: NextRequest) {
  ...
  const [{ data: own, error: ownError }, { data: relations, error: relationError }] = await Promise.all([
    supabaseAdmin.from("review_snapshots").select("id,rating,review_count,created_at")
  ...
  const [{ data: competitors, error: competitorError }, { data: snapshots, error: snapshotError }] = await Promise.all([
    ...
    supabaseAdmin.from("competitor_snapshots").select("id,competitor_id,rating,review_count,created_at")
      .in("competitor_id", ids).order("created_at", { ascending: false }).limit(500),
  ]);
